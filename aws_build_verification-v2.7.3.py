...
Changelog:
- v2.7.3: Improved CloudTrail verification for organization/Control Tower trails
  - Recognizes and reports organization trails properly
- v2.7.2: Fixed CIS benchmark crash on PolicyName lookup
  - PolicyName is nested in PolicyMetadata, not at top level
  - Added safe fallback for policy name extraction
- v2.7.1: SSO-aware password policy checks
- v2.7.0: Comprehensive CIS AWS Foundations Benchmark v1.4.0 (54 controls)
...
aws s3api put-bucket-logging --bucket {bucket_name} --bucket-logging-status '{{"LoggingEnabled": {{"TargetBucket": "YOUR-LOGGING-BUCKET", "TargetPrefix": "{bucket_name}/"}}}}' ''',
  ...
  target_prefix = "{bucket_name}/"
    ...
    def verify_cis_benchmarks(self) -> Dict[str, Any]:
        ...
        # Check Organizations and SSO context for password policy assessment
        org_data = self.collected_data.get('Organizations', {})
        is_member_account = org_data.get('IsMemberAccount', False)
        
        # Detect SSO usage by checking for AWSReservedSSO roles
        iam_roles = iam_data.get('Roles', [])
        sso_roles = [r for r in iam_roles if r.get('RoleName', '').startswith('AWSReservedSSO_')]
        sso_managed = len(sso_roles) > 0 or org_data.get('SSOEnabled', False)
        ...
        if pw_policy.get('Exists'):
            ...
            # Adjust severity based on SSO usage
            if sso_managed and is_member_account:
                severity = 'LOW'
                finding_suffix = ' (SSO-managed account - passwords managed centrally via Identity Center)'
                recommendation = f'⚪ LOW: Account uses AWS SSO/Identity Center ({len(sso_roles)} SSO roles detected). IAM password policy not applicable if no IAM users with passwords exist.'
            elif is_member_account:
                severity = 'MEDIUM'
                finding_suffix = ' (member account in Organization)'
                recommendation = '⚠️ MEDIUM: Member account without password policy. Verify if SSO is used or configure password policy.'
            else:
                severity = 'HIGH'
                finding_suffix = ''
            ...
            for cis_id in ['1.5', '1.6', '1.7', '1.8', '1.9', '1.10', '1.11']:
                results['checks'].append({
                    ...
                    'Status': 'FAIL',
                    'Severity': severity,
                    'Finding': f'No password policy configured{finding_suffix}',
        ...
        for policy in policies:
            # PolicyName might be at top level or nested in PolicyMetadata
            policy_name = policy.get('PolicyName') or policy.get('PolicyMetadata', {}).get('PolicyName') or 'Unknown'
        ...
        if multi_region_trails:
            finding = f'{len(multi_region_trails)} multi-region trail(s) active'
            if org_trails:
                finding += f' (includes {len(org_trails)} organization trail(s))'
            results['checks'].append({
                'BenchmarkID': 'CIS-3.1',
            ...
            results['checks'].append({
                ...
                'Status': 'FAIL',
                'Severity': 'CRITICAL',
                'Finding': 'No multi-region CloudTrail enabled',
