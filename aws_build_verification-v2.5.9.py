...
class BuildVerifier:
    ...
    def verify_databases(self) -> Dict[str, Any]:
        """Verify RDS, ElastiCache, and OpenSearch configurations"""
        results = {
    ...
    def verify_dynamodb(self) -> Dict[str, Any]:
        """Verify DynamoDB table configurations"""
        results = {
            'title': 'DynamoDB Tables Verification',
            'checks': []
        }
        ...
        for table in tables:
            ...
            # - If SSEDescription is missing or Status != ENABLED: AWS owned key (default, always encrypted)
            # - If SSEDescription.Status = ENABLED: Either AWS managed key or customer managed KMS
            sse = table.get('SSEDescription', {})
            sse_status = sse.get('Status', '')
            
            # Determine encryption type
            if sse_status == 'ENABLED':
                # Explicit encryption configured (AWS managed or customer KMS)
                encryption_type = sse.get('SSEType', 'KMS')  # Should be 'KMS'
                encryption_enabled = True
            else:
                # No SSEDescription or Status != ENABLED means AWS owned key (default encryption)
                encryption_type = 'AWS_OWNED'
                encryption_enabled = True  # AWS owned keys are always encrypted!
            ...
            # All DynamoDB tables are encrypted at rest by default (AWS owned keys)
            # We check for best practices: customer-managed KMS > AWS managed KMS > AWS owned
            if encryption_type == 'AWS_OWNED':
                ...
                if 'aws/dynamodb' in kms_key.lower() or not kms_key:
                    ...
                    if severity == 'INFO':
                        severity = 'LOW'
                # else: customer-managed KMS key - best practice, no issue
            ...
            if table_size >= 1073741824:  # GB
                size_display = f"{table_size / 1073741824:.2f} GB"
            elif table_size >= 1048576:  # MB
...
aws s3api put-bucket-logging --bucket {bucket_name} --bucket-logging-status '{{"LoggingEnabled": {{"TargetBucket": "YOUR-LOGGING-BUCKET", "TargetPrefix": "{bucket_name}/"}}}}' ''',
  ...
  target_prefix = "{bucket_name}/"
        ...
        for role_data in roles:
            ...
            if all_issues or max_severity != 'INFO':
                results['checks'].append({
                    ...
                    'Issues': all_issues if all_issues else ['Role has permissions - review recommended'],
    ...
    def verify_cis_benchmarks(self) -> Dict[str, Any]:
        ...
        # Root MFA
        if not root_checks.get('MFAActive', False):
            results['checks'].append({
                ...
                'Recommendation': '🔒 CRITICAL: Enable MFA for root account'
            })
