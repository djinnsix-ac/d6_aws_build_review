...
Changelog:
- v2.16.0: Added Djinn Six favicon and CIS control descriptions for better user understanding
- v2.15.0: Enhanced CIS section with comprehensive 54 controls and collapsible sections
- v2.14.0: FEATURE - IAM custom policy display with security analysis and clickable policy documents
- v2.13.19: Fixed Windows encoding error - use UTF-8 when writing HTML file
- v2.13.18: Moved DynamoDB tables into Databases section (with RDS and OpenSearch)
- v2.13.17: Fixed DynamoDB encryption display - show AWS owned keys as encrypted (green)
- v2.13.16: Added DynamoDB Tables section with security status display
- v2.13.15: Enhanced OpenSearch display, fixed accessibility and keyboard navigation
- v2.13.11: Fixed list alignment (wrap each item in span, no whitespace before <br>), formatted Access Policies as proper JSON
- v2.13.10: Database refinements: fixed leading spaces in subnets/SGs, sorted AZs alphabetically, "Number of Nodes" for OpenSearch, uniform KMS font, security warnings for missing encryption, clickable Access Policies
- v2.13.9: Enhanced RDS display (30+ fields), added OpenSearch domains section
- v2.13.8: ALB Target Groups 40%→25%, VPC wider, ECS services table: rename to Target Group, add Load Balancer column
- v2.13.7: ECS cluster names now h5 collapsible, adjusted LB table columns, AZs on separate lines, clickable target groups
- v2.13.6: Styled Lambda/ECS headers to match VPC (blue, underline), added Load Balancers section
- v2.13.5: Added JavaScript for h4.collapsible elements, adjusted service name to 28% and load balancer to 32%
- v2.13.4: Fixed Lambda/ECS to h4 (match VPC style), fixed load balancer extraction from camelCase fields
- v2.13.3: Lambda/ECS collapsible h5, VPC ✅/❌, service table 40% name width, clarify task counts, add load balancer info
- v2.13.2: Fixed Lambda table column widths (Function Name 40%), display VPC ID when in VPC, fixed ECS not displaying
- v2.13.1: Fixed ECS/EKS field names to match verification output (ServiceDetails not Services, ClusterName not Name)
- v2.13.0: Added ECS and EKS to Compute section - display everything we collect
- v2.12.13: Moved Network ACLs back under individual VPCs at h5 level (same as Route Tables) since they're VPC-specific
- v2.12.12: Fixed VPC Endpoints and VPC Peering to h5 level (same size as Route Tables/Subnet Distribution)
- v2.12.11: Made VPC name collapsible, removed Assocs column, widened VPC ID column, fixed NACL display to show all NACLs across all VPCs in one section
- v2.12.10: Fixed NACL count display, added VPC ID and Owner ID columns to NACL table
...
<head>
    ...
    <title>AWS Build Review Report - {account_id} | Djinn Six Limited</title>
    <link rel="icon" type="image/x-icon" href="data:image/x-icon;base64,">
    ...
    <style>
        * {{
        ...
        /* Lambda Functions table - wider function name */
        .lambda-table {{
            table-layout: fixed;
        }}
        
        .lambda-table th:nth-child(1), .lambda-table td:nth-child(1) {{ width: 40%; }} /* Function Name */
        .lambda-table th:nth-child(2), .lambda-table td:nth-child(2) {{ width: 15%; }} /* Runtime */
        .lambda-table th:nth-child(3), .lambda-table td:nth-child(3) {{ width: 12%; }} /* Memory */
        .lambda-table th:nth-child(4), .lambda-table td:nth-child(4) {{ width: 12%; }} /* Timeout */
        .lambda-table th:nth-child(5), .lambda-table td:nth-child(5) {{ width: 21%; }} /* VPC */
        
        /* ECS Services table - wider service name */
        .ecs-services-table {{
            table-layout: fixed;
        }}
        
        .ecs-services-table th:nth-child(1), .ecs-services-table td:nth-child(1) {{ width: 25%; }} /* Service Name */
        .ecs-services-table th:nth-child(2), .ecs-services-table td:nth-child(2) {{ width: 10%; }} /* Status */
        .ecs-services-table th:nth-child(3), .ecs-services-table td:nth-child(3) {{ width: 10%; }} /* Desired Tasks */
        .ecs-services-table th:nth-child(4), .ecs-services-table td:nth-child(4) {{ width: 10%; }} /* Running Tasks */
        .ecs-services-table th:nth-child(5), .ecs-services-table td:nth-child(5) {{ width: 10%; }} /* Launch Type */
        .ecs-services-table th:nth-child(6), .ecs-services-table td:nth-child(6) {{ width: 15%; }} /* Load Balancer */
        .ecs-services-table th:nth-child(7), .ecs-services-table td:nth-child(7) {{ width: 20%; }} /* Target Group */
        ...
        .alb-table th:nth-child(3), .alb-table td:nth-child(3) {{ width: 25%; }} /* VPC */
        ...
        .alb-table th:nth-child(6), .alb-table td:nth-child(6) {{ width: 30%; }} /* Target Groups */
...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            ...
            // Handle h4.collapsible elements (Lambda, ECS, etc.)
            document.querySelectorAll('h4.collapsible').forEach(h4 => {{
                h4.style.cursor = 'pointer';
                
                // Find the next sibling (should be collapsible-content div)
...
def generate_vpc_section(vpc_data, collected_data=None):
    ...
    for vpc_id, checks in vpc_checks.items():
        ...
        # VPC heading at h4 level - collapsible like other h4 sections
        vpc_content_id = f"vpc-content-{vpc_id}"
        html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('{vpc_content_id}')">
            <span id="{vpc_content_id}-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            VPC: {vpc_name}
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">({vpc_id})</span>
        </h4>
        '''
        
        html += f'<div id="{vpc_content_id}" style="display: none;">\n'
        ...
        # VPC Endpoints Section (h5 collapsible - same level as Route Tables)
        if raw_vpc:
        ...
        # VPC Peering Connections Section (h5 collapsible - same level as Route Tables)
        if raw_vpc:
        ...
        # Network ACLs Section (h5 collapsible - same level as Route Tables)
        if raw_vpc:
            ...
            if network_acls:
                ...
                html += '<table>\n'
                html += '<thead><tr>'
                html += '<th style="width: 22%;">NACL ID</th>'
                html += '<th style="width: 10%;">Default</th>'
                html += '<th style="width: 10%;">Inbound</th>'
                html += '<th style="width: 10%;">Outbound</th>'
                html += '<th style="width: 48%;">Associated Subnets</th>'
                html += '</tr></thead>\n'
                html += '<tbody>\n'
                ...
                for idx, nacl in enumerate(network_acls):
                    ...
                    html += f'''<tr id="nacl-details-{vpc_id}-{idx}" style="display:none;">
                        <td colspan="5">{generate_nacl_details(nacl)}</td>
                    </tr>\n'''
                    ...
                    for assoc in associations:
                        if '_full_subnet' in assoc and '_detail_id' in assoc:
                            ...
                            full_subnet = assoc['_full_subnet']
                            html += f'''<tr id="{detail_id}" style="display:none;">
                                <td colspan="5">{generate_subnet_details(full_subnet)}</td>
                            </tr>\n'''
                
                html += '</tbody></table>\n'
                html += '</div>\n'  # Close Network ACLs collapsible section
        
        html += '</div>\n'  # Close VPC content collapsible div
...
'''
    ...
    for idx, sg in enumerate(sg_data.get('checks', [])):
        ...
        # Use new OpenToInternetCount if available, fall back to old OpenToInternet
        open_count = sg.get('OpenToInternetCount', len(sg.get('OpenToInternet', [])))
        open_text = f"{open_count} rules" if open_count > 0 else "None"
        ...
        if ingress_count > 0:
            ...
            html += f'''<tr id="sg-ingress-{idx}" class="rules-expanded-row" style="display:none;">
                <td colspan="7">
                    <div class="rules-details">
                        <h4>📥 Ingress Rules ({ingress_count}) - Click rule count to collapse</h4>
                        {ingress_rules_html}
                    </div>
                </td>
            </tr>\n'''
        ...
        if egress_count > 0:
            ...
            html += f'''<tr id="sg-egress-{idx}" class="rules-expanded-row" style="display:none;">
                <td colspan="7">
                    <div class="rules-details">
                        <h4>📤 Egress Rules ({egress_count}) - Click rule count to collapse</h4>
                        {egress_rules_html}
                    </div>
                </td>
            </tr>\n'''
...
def generate_compute_section(compute_data):
    ...
    if compute_data.get('lambda'):
        lambda_count = len(compute_data['lambda'])
        html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('lambda-content')">
            <span id="lambda-content-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            Lambda Functions
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">({lambda_count})</span>
        </h4>
        '''
        html += '<div id="lambda-content" style="display: none;">\n'
        html += '<table class="lambda-table">\n'
        ...
        for func in compute_data['lambda']:
            vpc = func.get('VPC', '')
            vpc_display = f'✅ <span class="code">{vpc}</span>' if vpc else '❌'
    ...
    if compute_data.get('ecs'):
        cluster_count = len(compute_data['ecs'])
        html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('ecs-content')">
            <span id="ecs-content-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            ECS Clusters
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">({cluster_count})</span>
        </h4>
        '''
        html += '<div id="ecs-content" style="display: none;">\n'
        
        for cluster in compute_data['ecs']:
            cluster_name = cluster.get('ClusterName', 'Unknown')
            cluster_id = cluster_name.replace(' ', '-').replace('_', '-')
            
            # Cluster heading as h5 collapsible
            html += f'''<h5 style="margin-top: 20px; color: #2c5aa0; font-size: 15px; cursor: pointer; padding: 8px 0;" 
                onclick="toggleSection('cluster-{cluster_id}')">
                <span id="cluster-{cluster_id}-icon" style="display: inline-block; width: 20px;">&#9658;</span>
                Cluster: {cluster_name}
            </h5>\n'''
            
            html += f'<div id="cluster-{cluster_id}" style="display: none;">\n'
            
            # Cluster info table
            html += '<table>\n'
            html += '<thead><tr><th>Property</th><th>Value</th></tr></thead>\n'
            html += '<tbody>\n'
            html += f'<tr><td>Status</td><td>{cluster.get("Status", "")}</td></tr>\n'
            html += f'<tr><td>Registered Instances</td><td>{cluster.get("RegisteredInstances", 0)}</td></tr>\n'
            html += f'<tr><td>Running Tasks</td><td>{cluster.get("RunningTasks", 0)}</td></tr>\n'
            html += f'<tr><td>Active Services</td><td>{cluster.get("Services", 0)}</td></tr>\n'
            html += '</tbody></table>\n'
            
            # Services in this cluster
            services = cluster.get('ServiceDetails', [])
            if services and isinstance(services, list):
                html += '<h5>Services</h5>\n'
                html += '<table class="ecs-services-table">\n'
                html += '<thead><tr><th>Service Name</th><th>Status</th><th>Desired Tasks</th><th>Running Tasks</th><th>Launch Type</th><th>Load Balancer</th><th>Target Group</th></tr></thead>\n'
                html += '<tbody>\n'
                
                for service in services:
                    status = service.get('Status', '')
                    badge = 'badge-success' if status == 'ACTIVE' else 'badge-warning'
                    
                    # Get load balancer name
                    lb_name = service.get('LoadBalancerName', '')
                    lb_display = f'<span class="code">{lb_name}</span>' if lb_name else '—'
                    
                    # Get target group info - check both camelCase and PascalCase
                    tg_info = ''
                    ...
                    if load_balancers and len(load_balancers) > 0:
                        ...
                        if tg_arn:
                            ...
                            tg_info = f'{tg_name}:{container_port}' if container_port else tg_name
                    
                    html += f'''<tr>
                        <td><span class="code">{service.get("ServiceName", "")}</span></td>
                        <td><span class="badge {badge}">{status}</span></td>
                        <td>{service.get("DesiredCount", 0)}</td>
                        <td>{service.get("RunningCount", 0)}</td>
                        <td>{service.get("LaunchType", "")}</td>
                        <td>{lb_display}</td>
                        <td><span class="code">{tg_info if tg_info else '—'}</span></td>
                    </tr>\n'''
        ...
        html += '</div>\n'  # Close ecs-content div
    
    # EKS Clusters
    if compute_data.get('eks'):
        html += '<h3>EKS Clusters</h3>\n'
        html += '<table>\n'
        html += '<thead><tr><th>Cluster Name</th><th>Status</th><th>Version</th><th>Endpoint</th><th>Platform</th><th>VPC</th></tr></thead>\n'
        html += '<tbody>\n'
        
        for cluster in compute_data['eks']:
            status = cluster.get('Status', '')
            badge = 'badge-success' if status == 'ACTIVE' else 'badge-warning'
            
            html += f'''<tr>
                <td><span class="code">{cluster.get("ClusterName", "")}</span></td>
                <td><span class="badge {badge}">{status}</span></td>
                <td>{cluster.get("Version", "")}</td>
                <td><span class="code">{cluster.get("Endpoint", "")}</span></td>
                <td>N/A</td>
                <td><span class="code">{cluster.get("VPC", "")}</span></td>
            </tr>\n'''
...
def generate_database_section(db_data):
    """Generate HTML for database section with enhanced RDS and OpenSearch"""
    if not db_data:
        return ""
    ...
    # RDS Instances - Enhanced with collapsible details
    if db_data.get('rds'):
        rds_count = len(db_data['rds'])
        html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('rds-content')">
            <span id="rds-content-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            RDS Instances
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">({rds_count})</span>
        </h4>
        '''
        html += '<div id="rds-content" style="display: none;">\n'
        ...
        for db in db_data['rds']:
            ...
            # Main summary
            html += f'''<h5 style="margin-top: 20px; color: #2c5aa0; font-size: 15px; cursor: pointer; padding: 8px 0;" 
                onclick="toggleSection('rds-{db_id}')">
                <span id="rds-{db_id}-icon" style="display: inline-block; width: 20px;">&#9658;</span>
                {db_id}
            </h5>\n'''
            
            html += f'<div id="rds-{db_id}" style="display: none;">\n'
            html += '<table>\n'
            html += '<thead><tr><th style="width: 30%;">Property</th><th>Value</th></tr></thead>\n'
            html += '<tbody>\n'
            
            # Basic Info
            html += f'<tr><td><strong>Database Name</strong></td><td>{db.get("DBName", "N/A")}</td></tr>\n'
            html += f'<tr><td><strong>Engine</strong></td><td>{db.get("Engine", "")} {db.get("EngineVersion", "")}</td></tr>\n'
            html += f'<tr><td><strong>Instance Class</strong></td><td>{db.get("InstanceClass", "")}</td></tr>\n'
            
            # Endpoint
            endpoint = db.get('Endpoint')
            port = db.get('Port')
            if endpoint:
                html += f'<tr><td><strong>Endpoint</strong></td><td><span class="code">{endpoint}:{port}</span></td></tr>\n'
            
            # Availability
            multi_az = '✅ Yes' if db.get('MultiAZ') else '❌ No'
            html += f'<tr><td><strong>Multi-AZ</strong></td><td>{multi_az}</td></tr>\n'
            html += f'<tr><td><strong>Availability Zone</strong></td><td>{db.get("AvailabilityZone", "N/A")}</td></tr>\n'
            
            # Storage
            html += f'<tr><td><strong>Storage Type</strong></td><td>{db.get("StorageType", "")}</td></tr>\n'
            html += f'<tr><td><strong>Allocated Storage</strong></td><td>{db.get("AllocatedStorage", 0)} GB</td></tr>\n'
            if db.get('MaxAllocatedStorage'):
                html += f'<tr><td><strong>Max Allocated Storage</strong></td><td>{db.get("MaxAllocatedStorage")} GB</td></tr>\n'
            if db.get('Iops'):
                html += f'<tr><td><strong>IOPS</strong></td><td>{db.get("Iops")}</td></tr>\n'
            if db.get('StorageThroughput'):
                html += f'<tr><td><strong>Throughput</strong></td><td>{db.get("StorageThroughput")} MB/s</td></tr>\n'
            
            # Security
            encrypted = '✅ Yes' if db.get('StorageEncrypted') else '❌ No'
            html += f'<tr><td><strong>Storage Encrypted</strong></td><td>{encrypted}</td></tr>\n'
            if db.get('KmsKeyId'):
                html += f'<tr><td><strong>KMS Key</strong></td><td><span class="code">{db.get("KmsKeyId")}</span></td></tr>\n'
            
            public = db.get('PubliclyAccessible')
            public_display = '⚠️ Yes' if public else '✅ No'
            public_badge = 'badge-danger' if public else 'badge-success'
            html += f'<tr><td><strong>Publicly Accessible</strong></td><td><span class="badge {public_badge}">{public_display}</span></td></tr>\n'
            
            iam_auth = '✅ Enabled' if db.get('IAMDatabaseAuthentication') else '❌ Disabled'
            html += f'<tr><td><strong>IAM Authentication</strong></td><td>{iam_auth}</td></tr>\n'
            
            deletion_protection = '✅ Enabled' if db.get('DeletionProtection') else '❌ Disabled'
            html += f'<tr><td><strong>Deletion Protection</strong></td><td>{deletion_protection}</td></tr>\n'
            
            # Backup
            html += f'<tr><td><strong>Backup Retention</strong></td><td>{db.get("BackupRetention", 0)} days</td></tr>\n'
            if db.get('PreferredBackupWindow'):
                html += f'<tr><td><strong>Backup Window</strong></td><td>{db.get("PreferredBackupWindow")}</td></tr>\n'
            if db.get('LatestRestorableTime'):
                html += f'<tr><td><strong>Latest Restorable Time</strong></td><td>{db.get("LatestRestorableTime")}</td></tr>\n'
            
            # Monitoring
            perf_insights = '✅ Enabled' if db.get('PerformanceInsightsEnabled') else '❌ Disabled'
            html += f'<tr><td><strong>Performance Insights</strong></td><td>{perf_insights}</td></tr>\n'
            
            monitoring_interval = db.get('MonitoringInterval', 0)
            monitoring_display = f'{monitoring_interval} seconds' if monitoring_interval > 0 else 'Disabled'
            html += f'<tr><td><strong>Enhanced Monitoring</strong></td><td>{monitoring_display}</td></tr>\n'
            
            # Maintenance
            if db.get('PreferredMaintenanceWindow'):
                html += f'<tr><td><strong>Maintenance Window</strong></td><td>{db.get("PreferredMaintenanceWindow")}</td></tr>\n'
            
            auto_upgrade = '✅ Enabled' if db.get('AutoMinorVersionUpgrade') else '❌ Disabled'
            html += f'<tr><td><strong>Auto Minor Version Upgrade</strong></td><td>{auto_upgrade}</td></tr>\n'
            
            # Network
            html += f'<tr><td><strong>VPC</strong></td><td><span class="code">{db.get("VPC", "N/A")}</span></td></tr>\n'
            if db.get('SubnetGroup'):
                html += f'<tr><td><strong>Subnet Group</strong></td><td>{db.get("SubnetGroup")}</td></tr>\n'
            
            subnets = db.get('Subnets', [])
            if subnets:
                # Join without any whitespace between items and <br> tags
                subnet_items = [f'<span class="code">{s.strip()}</span>' for s in sorted(subnets)]
                subnet_list = '<br>'.join(subnet_items)
                html += f'<tr><td><strong>Subnets</strong></td><td>{subnet_list}</td></tr>\n'
            
            security_groups = db.get('SecurityGroups', [])
            if security_groups:
                # Join without any whitespace between items and <br> tags
                sg_items = [f'<span class="code">{sg.strip()}</span>' for sg in sorted(security_groups)]
                sg_list = '<br>'.join(sg_items)
                html += f'<tr><td><strong>Security Groups</strong></td><td>{sg_list}</td></tr>\n'
            
            html += '</tbody></table>\n'
            html += '</div>\n'  # Close instance div
        
        html += '</div>\n'  # Close RDS content div
    
    # OpenSearch Domains
    if db_data.get('opensearch'):
        os_count = len(db_data['opensearch'])
        html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('opensearch-content')">
            <span id="opensearch-content-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            OpenSearch Domains
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">({os_count})</span>
        </h4>
        '''
        html += '<div id="opensearch-content" style="display: none;">\n'
        
        for domain in db_data['opensearch']:
            domain_name = domain.get('DomainName', 'unknown')
            domain_id_safe = domain_name.replace('-', '_').replace('.', '_')
            
            html += f'''<h5 style="margin-top: 20px; color: #2c5aa0; font-size: 15px; cursor: pointer; padding: 8px 0;" 
                onclick="toggleSection('os-{domain_id_safe}')">
                <span id="os-{domain_id_safe}-icon" style="display: inline-block; width: 20px;">&#9658;</span>
                {domain_name}
            </h5>\n'''
            
            html += f'<div id="os-{domain_id_safe}" style="display: none;">\n'
            html += '<table>\n'
            html += '<thead><tr><th style="width: 30%;">Property</th><th>Value</th></tr></thead>\n'
            html += '<tbody>\n'
            
            # Basic Info
            html += f'<tr><td><strong>Domain ID</strong></td><td><span class="code">{domain.get("DomainId", "")}</span></td></tr>\n'
            html += f'<tr><td><strong>Engine Version</strong></td><td>{domain.get("EngineVersion", "")}</td></tr>\n'
            
            endpoint = domain.get('Endpoint')
            if endpoint:
                html += f'<tr><td><strong>Endpoint</strong></td><td><span class="code">{endpoint}</span></td></tr>\n'
            
            # Cluster Configuration
            html += f'<tr><td><strong>Instance Type</strong></td><td>{domain.get("InstanceType", "")}</td></tr>\n'
            html += f'<tr><td><strong>Number of Nodes</strong></td><td>{domain.get("InstanceCount", 0)}</td></tr>\n'
            
            dedicated_master = '✅ Yes' if domain.get('DedicatedMasterEnabled') else '❌ No'
            html += f'<tr><td><strong>Dedicated Master</strong></td><td>{dedicated_master}</td></tr>\n'
            
            zone_aware = '✅ Yes' if domain.get('ZoneAwarenessEnabled') else '✅ Yes'
            html += f'<tr><td><strong>Zone Awareness</strong></td><td>{zone_aware}</td></tr>\n'
            
            # Security - with warnings for vector databases
            encrypt_rest_enabled = domain.get('EncryptionAtRestEnabled')
            if encrypt_rest_enabled:
                encrypt_rest = '✅ Enabled'
                encrypt_rest_badge = 'badge-success'
            else:
                encrypt_rest = '⚠️ Disabled'
                encrypt_rest_badge = 'badge-danger'
            html += f'<tr><td><strong>Encryption at Rest</strong></td><td><span class="badge {encrypt_rest_badge}">{encrypt_rest}</span></td></tr>\n'
            if not encrypt_rest_enabled:
                html += '<tr><td colspan="2" style="background: #fff3cd; padding: 8px;"><strong>⚠️ Security Warning:</strong> Encryption at rest is disabled. For vector databases storing embeddings and sensitive data, encryption at rest is strongly recommended to protect data confidentiality.</td></tr>\n'
            
            encrypt_node_enabled = domain.get('NodeToNodeEncryptionEnabled')
            if encrypt_node_enabled:
                encrypt_node = '✅ Enabled'
                encrypt_node_badge = 'badge-success'
            else:
                encrypt_node = '⚠️ Disabled'
                encrypt_node_badge = 'badge-danger'
            html += f'<tr><td><strong>Node-to-Node Encryption</strong></td><td><span class="badge {encrypt_node_badge}">{encrypt_node}</span></td></tr>\n'
            if not encrypt_node_enabled:
                html += '<tr><td colspan="2" style="background: #fff3cd; padding: 8px;"><strong>⚠️ Security Warning:</strong> Node-to-node encryption is disabled. This leaves inter-node communication unencrypted within the cluster, which could expose sensitive data in transit. This should be enabled for production workloads.</td></tr>\n'
            
            # Access Policies - clickable with formatted JSON
            access_policies = domain.get('AccessPolicies')
            if access_policies:
                # Format the JSON policy for display
                import json
                try:
                ...
                html += f'''<tr><td><strong>Access Policies</strong></td><td>
                    <span style="color: #667eea; cursor: pointer; text-decoration: underline;" 
                          onclick="toggleDetails('os-{domain_id_safe}-policies')">
                        View Policies
                    </span>
                    <div id="os-{domain_id_safe}-policies" style="display: none; margin-top: 8px; padding: 12px; background: #f8f8f8; border: 1px solid #ddd; border-radius: 4px; font-family: 'Courier New', monospace; font-size: 12px; white-space: pre; overflow-x: auto;">
{formatted_policy}
                    </div>
                </td></tr>\n'''
            ...
            if vpc_id:
                html += f'<tr><td><strong>VPC</strong></td><td><span class="code">{vpc_id}</span></td></tr>\n'
                
                subnets = domain.get('SubnetIds', [])
                if subnets:
                    # Join without any whitespace between items and <br> tags
                    subnet_items = [f'<span class="code">{s.strip()}</span>' for s in sorted(subnets)]
                    subnet_list = '<br>'.join(subnet_items)
                ...
                security_groups = domain.get('SecurityGroupIds', [])
                if security_groups:
                    # Join without any whitespace between items and <br> tags
                    sg_items = [f'<span class="code">{sg.strip()}</span>' for sg in sorted(security_groups)]
                    sg_list = '<br>'.join(sg_items)
                ...
                azs = domain.get('AvailabilityZones', [])
                if azs:
                    # Sort availability zones alphabetically
                    az_list = '<br>'.join(sorted(azs))
                    html += f'<tr><td><strong>Availability Zones</strong></td><td>{az_list}</td></tr>\n'
            
            # Status
            processing = '⚠️ Yes' if domain.get('Processing') else '✅ No'
            html += f'<tr><td><strong>Processing</strong></td><td>{processing}</td></tr>\n'
            
            upgrading = '⚠️ Yes' if domain.get('UpgradeProcessing') else '✅ No'
            html += f'<tr><td><strong>Upgrade Processing</strong></td><td>{upgrading}</td></tr>\n'
            
            html += '</tbody></table>\n'
            html += '</div>\n'  # Close domain div
        
        html += '</div>\n'  # Close OpenSearch content div
    
    # DynamoDB Tables
    if db_data.get('dynamodb'):
        dynamodb_checks = db_data['dynamodb'].get('checks', [])
        if dynamodb_checks:
            dynamo_count = len(dynamodb_checks)
            html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('dynamodb-content')">
            <span id="dynamodb-content-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            DynamoDB Tables
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">({dynamo_count})</span>
        </h4>
        '''
            html += '<div id="dynamodb-content" style="display: none;">\n'
            
            # Count issues by severity for alert banner
            high_risk = sum(1 for table in dynamodb_checks if table.get('Severity') == 'HIGH')
            medium_risk = sum(1 for table in dynamodb_checks if table.get('Severity') == 'MEDIUM')
            low_risk = sum(1 for table in dynamodb_checks if table.get('Severity') == 'LOW')
            
            # Alert banner (smaller, inline with section)
            if high_risk > 0:
                html += f'<div class="alert alert-danger" style="margin: 10px 0;">⚠️ {high_risk} table(s) with HIGH severity issues</div>\n'
            elif medium_risk > 0:
                html += f'<div class="alert alert-warning" style="margin: 10px 0;">⚠️ {medium_risk} table(s) with MEDIUM severity issues</div>\n'
            elif low_risk > 0:
                html += f'<div class="alert alert-info" style="margin: 10px 0;">ℹ️ {low_risk} table(s) with recommendations</div>\n'
            
            # Summary statistics
            total_items = sum(table.get('ItemCount', 0) for table in dynamodb_checks)
            total_size_bytes = sum(table.get('TableSize', 0) for table in dynamodb_checks)
            
            if total_size_bytes >= 1073741824:
                total_size = f"{total_size_bytes / 1073741824:.2f} GB"
            elif total_size_bytes >= 1048576:
                total_size = f"{total_size_bytes / 1048576:.2f} MB"
            elif total_size_bytes >= 1024:
                total_size = f"{total_size_bytes / 1024:.2f} KB"
            else:
                total_size = f"{total_size_bytes} bytes"
            
            html += f'<p style="margin: 10px 0;"><strong>Summary:</strong> {dynamo_count} table(s) | {total_items:,} total items | {total_size} total size</p>\n'
            
            # Display each table
            for table in dynamodb_checks:
                table_name = table.get('TableName', 'unknown')
                table_id_safe = table_name.replace('-', '_').replace('.', '_')
                severity = table.get('Severity', 'INFO')
                
                html += f'''<h5 style="margin-top: 20px; color: #2c5aa0; font-size: 15px; cursor: pointer; padding: 8px 0;" 
                    onclick="toggleSection('dynamo-{table_id_safe}')">
                    <span id="dynamo-{table_id_safe}-icon" style="display: inline-block; width: 20px;">&#9658;</span>
                    {table_name}
                    <span class="badge badge-{severity.lower()}" style="margin-left: 10px;">{severity}</span>
                </h5>\n'''
                
                html += f'<div id="dynamo-{table_id_safe}" style="display: none;">\n'
                html += '<table>\n'
                html += '<thead><tr><th style="width: 30%;">Property</th><th>Value</th></tr></thead>\n'
                html += '<tbody>\n'
                ...
                html += f'<tr><td><strong>Status</strong></td><td><span class="{status_class}">{status}</span></td></tr>\n'
                
                table_arn = table.get('TableArn', '')
                if table_arn:
                    html += f'<tr><td><strong>ARN</strong></td><td><span class="code">{table_arn}</span></td></tr>\n'
                
                # Billing
                billing = table.get('BillingMode', 'Unknown')
                html += f'<tr><td><strong>Billing Mode</strong></td><td>{billing}</td></tr>\n'
                
                # Security - Encryption
                encryption = table.get('Encryption', 'Unknown')
                encryption_enabled = table.get('EncryptionEnabled', False)
                
                if encryption == 'AWS_OWNED':
                    encryption_display = 'AWS Owned Key'
                    encryption_badge = 'badge-success'
                elif encryption == 'KMS':
                    kms_key = table.get('KMSKeyId', 'N/A')
                    if 'aws/dynamodb' in kms_key.lower():
                        encryption_display = 'AWS Managed Key (KMS)'
                    else:
                        encryption_display = 'Customer Managed Key (KMS)'
                    encryption_badge = 'badge-success'
                else:
                    encryption_display = encryption
                    encryption_badge = 'badge-danger' if not encryption_enabled else 'badge-success'
                
                html += f'<tr><td><strong>Encryption at Rest</strong></td><td><span class="badge {encryption_badge}">{encryption_display}</span></td></tr>\n'
                
                if table.get('KMSKeyId') and table.get('KMSKeyId') != 'N/A':
                    html += f'<tr><td><strong>KMS Key</strong></td><td><span class="code">{table.get("KMSKeyId")}</span></td></tr>\n'
                
                # Security - PITR
                pitr = table.get('PITR', 'Disabled')
                pitr_enabled = table.get('PITREnabled', False)
                pitr_display = '✅ Enabled' if pitr_enabled else '❌ Disabled'
                pitr_badge = 'badge-success' if pitr_enabled else 'badge-warning'
                html += f'<tr><td><strong>Point-in-Time Recovery</strong></td><td><span class="badge {pitr_badge}">{pitr_display}</span></td></tr>\n'
                
                # Security - Deletion Protection
                deletion_protection = table.get('DeletionProtection', 'No')
                deletion_protected = table.get('DeletionProtected', False)
                deletion_display = '✅ Enabled' if deletion_protected else '❌ Disabled'
                deletion_badge = 'badge-success' if deletion_protected else 'badge-warning'
                html += f'<tr><td><strong>Deletion Protection</strong></td><td><span class="badge {deletion_badge}">{deletion_display}</span></td></tr>\n'
                
                # Table Metrics
                item_count = table.get('ItemCount', 0)
                html += f'<tr><td><strong>Item Count</strong></td><td>{item_count:,}</td></tr>\n'
                
                size_display = table.get('TableSizeDisplay', '0 bytes')
                html += f'<tr><td><strong>Table Size</strong></td><td>{size_display}</td></tr>\n'
                
                # Tags
                if table.get('HasTags'):
                    tags_dict = table.get('Tags', {})
                    tags_display = ', '.join([f"{k}={v}" for k, v in tags_dict.items()])
                    html += f'<tr><td><strong>Tags</strong></td><td>{tags_display}</td></tr>\n'
                
                # Issues
                issues = table.get('Issues', [])
                if issues:
                    html += '<tr><td colspan="2" style="background: #fff3cd; padding: 10px;">\n'
                    html += '<strong>⚠️ Issues:</strong>\n'
...
def generate_load_balancer_section(lb_data):
    """Generate HTML for load balancers section"""
    if not lb_data or 'checks' not in lb_data:
        return ""
    
    lbs = lb_data.get('checks', [])
    if not lbs:
        return ""
    
    html = '<div class="section">\n'
    html += '<h2>Load Balancers</h2>\n'
    ...
    if albs:
        html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('alb-content')">
            <span id="alb-content-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            Application Load Balancers
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">({len(albs)})</span>
        </h4>
        '''
        html += '<div id="alb-content" style="display: none;">\n'
        html += '<table class="alb-table">\n'
        html += '<thead><tr><th>Name</th><th>Scheme</th><th>VPC</th><th>AZs</th><th>Listeners</th><th>Target Groups</th></tr></thead>\n'
        html += '<tbody>\n'
        
        for lb in albs:
            scheme = lb.get('Scheme', '')
            scheme_badge = 'badge-warning' if scheme == 'internet-facing' else 'badge-info'
            azs = '<br>'.join(lb.get('AvailabilityZones', []))
            
            # Format listeners
            listeners = lb.get('Listeners', [])
            listener_text = '<br>'.join([f"{l.get('Protocol')}:{l.get('Port')}" for l in listeners])
            
            # Format target groups with clickable details
            tgs = lb.get('TargetGroups', [])
            tg_list = []
            for idx, tg in enumerate(tgs):
                tg_name = tg.get('Name', '')
                healthy = tg.get('HealthyTargets', 0)
                total = tg.get('TotalTargets', 0)
                tg_id = f"alb-tg-{lb.get('LoadBalancerName', '')}-{idx}"
                
                # Target group details for expandable
                tg_details = f'''
                <div id="{tg_id}" style="display:none; margin-top:5px; padding:8px; background:#f5f5f5; border-radius:4px; font-size:11px;">
                    <strong>Protocol:</strong> {tg.get('Protocol', 'N/A')}<br>
                    <strong>Port:</strong> {tg.get('Port', 'N/A')}<br>
                    <strong>Health Check:</strong> {'Enabled' if tg.get('HealthCheckEnabled') else 'Disabled'}
                </div>
                '''
                
                tg_list.append(f'<span style="cursor:pointer; text-decoration:underline;" onclick="toggleDetails(\'{tg_id}\')">{tg_name}</span> ({healthy}/{total} healthy){tg_details}')
            
            tg_text = '<br>'.join(tg_list)
            
            html += f'''<tr>
                <td><span class="code">{lb.get("LoadBalancerName", "")}</span></td>
                <td><span class="badge {scheme_badge}">{scheme}</span></td>
                <td><span class="code">{lb.get("VPC", "")}</span></td>
                <td>{azs}</td>
                <td>{listener_text}</td>
                <td>{tg_text}</td>
            </tr>\n'''
    ...
    if nlbs:
        html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('nlb-content')">
            <span id="nlb-content-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            Network Load Balancers
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">({len(nlbs)})</span>
        </h4>
        '''
        html += '<div id="nlb-content" style="display: none;">\n'
        html += '<table class="nlb-table">\n'
        html += '<thead><tr><th>Name</th><th>Scheme</th><th>VPC</th><th>AZs</th><th>Listeners</th><th>Target Groups</th></tr></thead>\n'
        html += '<tbody>\n'
        ...
        for lb in nlbs:
            ...
            for idx, tg in enumerate(tgs):
                ...
                healthy = tg.get('HealthyTargets', 0)
                total = tg.get('TotalTargets', 0)
                tg_id = f"nlb-tg-{lb.get('LoadBalancerName', '')}-{idx}"
...
def generate_iam_section(iam_data):
    ...
    if role_checks:
        ...
        html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('iam-roles')">
            <span id="iam-roles-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            IAM Roles - Security Analysis
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">({role_count})</span>
        </h4>
        '''
        
        html += '<div id="iam-roles" style="display: none;">\n'
        html += '<table>\n'
    ...
    if user_checks:
        html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('iam-users')">
            <span id="iam-users-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            IAM Users
        </h4>
        '''
        
        html += '<div id="iam-users" style="display: none;">\n'
    ...
    policy_checks = [c for c in checks if c.get('ResourceType') == 'IAMCustomPolicy']
    if policy_checks:
        policy_count = len(policy_checks)
        critical_count = sum(1 for p in policy_checks if p.get('Severity') == 'CRITICAL')
        high_count = sum(1 for p in policy_checks if p.get('Severity') == 'HIGH')
        
        html += f'''
        <h4 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('custom-policies')">
            <span id="custom-policies-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            Customer Managed Policies
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">({policy_count})</span>
        </h4>
        '''
        
        html += '<div id="custom-policies" style="display: none;">\n'
        
        # Alert banner
        if critical_count > 0:
            html += f'<div class="alert alert-danger">🚨 Found {critical_count} policy/policies with CRITICAL security issues</div>\n'
        elif high_count > 0:
            html += f'<div class="alert alert-warning">⚠️ Found {high_count} policy/policies with HIGH severity issues</div>\n'
        ...
        for policy in policy_checks:
            ...
            statement_count = policy.get('StatementCount', 0)
            policy_size = policy.get('PolicySize', 0)
            policy_doc = policy.get('PolicyDocument', {})
            issues = policy.get('Issues', [])
            detailed_issues = policy.get('DetailedIssues', [])
            has_wildcard_actions = policy.get('HasWildcardActions', False)
            has_wildcard_resources = policy.get('HasWildcardResources', False)
            high_risk_actions = policy.get('HighRiskActions', [])
            
            # Create safe ID for HTML
            policy_id = policy_name.replace('-', '_').replace('.', '_').replace(':', '_')
            
            badge_class = f'badge-{severity.lower()}'
            
            html += f'''
            <h5 style="margin-top: 20px; color: #2c5aa0; font-size: 15px; cursor: pointer; padding: 8px 0;" 
                onclick="toggleSection('policy-{policy_id}')">
                <span id="policy-{policy_id}-icon" style="display: inline-block; width: 20px;">&#9658;</span>
                {policy_name}
                <span class="badge {badge_class}" style="margin-left: 10px;">{severity}</span>
            </h5>
            '''
            
            html += f'<div id="policy-{policy_id}" style="display: none;">\n'
            
            # Policy metadata table
            html += '<table>\n'
            html += '<thead><tr><th style="width: 30%;">Property</th><th>Value</th></tr></thead>\n'
            html += '<tbody>\n'
            html += f'<tr><td><strong>Policy ARN</strong></td><td><span class="code">{policy_arn}</span></td></tr>\n'
            html += f'<tr><td><strong>Statements</strong></td><td>{statement_count}</td></tr>\n'
            html += f'<tr><td><strong>Policy Size</strong></td><td>{policy_size:,} characters</td></tr>\n'
            
            # Risk indicators
            wildcard_status = '⚠️ Yes' if has_wildcard_actions else '✅ No'
            wildcard_class = 'text-warning' if has_wildcard_actions else 'text-success'
            html += f'<tr><td><strong>Uses Wildcard Actions</strong></td><td><span class="{wildcard_class}">{wildcard_status}</span></td></tr>\n'
            
            resource_status = '⚠️ Yes' if has_wildcard_resources else '✅ No'
            resource_class = 'text-warning' if has_wildcard_resources else 'text-success'
            html += f'<tr><td><strong>Uses Wildcard Resources</strong></td><td><span class="{resource_class}">{resource_status}</span></td></tr>\n'
            
            if high_risk_actions:
                actions_display = ', '.join(high_risk_actions)
                html += f'<tr><td><strong>High-Risk Actions</strong></td><td><span class="code">{actions_display}</span></td></tr>\n'
            ...
            if policy_doc:
                ...
                <div style="margin: 15px 0;">
                    <button style="padding: 8px 16px; background: #667eea; color: white; border: none; border-radius: 4px; cursor: pointer;"
                            onclick="toggleDetails('policy-doc-{policy_id}')">
                        📄 View Policy Document
                    </button>
                </div>
                <div id="policy-doc-{policy_id}" style="display: none; margin: 10px 0;">
                    <pre class="json-code" style="background: #f5f5f5; padding: 15px; border-radius: 4px; font-family: 'Courier New', monospace; font-size: 11px; overflow-x: auto; border: 1px solid #ddd; max-height: 400px;">{formatted_policy}</pre>
                </div>
                '''
            
            # Security Issues
            if detailed_issues:
                html += '<h6 style="margin-top: 20px; color: #d32f2f;">🔒 Security Issues</h6>\n'
                html += '<table class="issues-table">\n'
                html += '<thead><tr><th style="width: 15%;">Category</th><th style="width: 10%;">Severity</th><th style="width: 30%;">Finding</th><th style="width: 25%;">Impact</th><th style="width: 20%;">Recommendation</th></tr></thead>\n'
                html += '<tbody>\n'
                ...
                for issue in detailed_issues:
                    ...
                    # Row class based on severity
                    row_class = f'issue-{issue_severity.lower()}'
                    badge_class = f'badge-{issue_severity.lower()}'
                    
                    html += f'''<tr class="{row_class}">
                        <td><strong>{category}</strong></td>
                        <td><span class="badge {badge_class}">{issue_severity}</span></td>
                        <td>{finding}</td>
                        <td>{impact}</td>
                        <td>{recommendation}</td>
                    </tr>\n'''
                ...
                # Fallback to simple issues list
                html += '<div style="background: #fff3cd; padding: 10px; margin: 10px 0; border-radius: 4px;">\n'
                html += '<strong>⚠️ Issues:</strong><ul style="margin: 5px 0 0 20px;">\n'
                for issue in issues:
                    html += f'<li>{issue}</li>\n'
                html += '</ul></div>\n'
...
def get_cis_control_description(benchmark_id):
    """Return user-friendly description for each CIS control"""
    descriptions = {
        # Section 1: Identity and Access Management
        'CIS-1.4': 'Root account access keys should not exist - root credentials should never be programmatic',
        'CIS-1.5': 'MFA should be enabled on root account for critical security protection',
        'CIS-1.6': 'Hardware MFA on root account provides stronger security than virtual MFA',
        'CIS-1.7': 'Eliminate unnecessary IAM user permissions that are unused within 45 days',
        'CIS-1.8': 'Password policies should enforce minimum length of 14 characters or more',
        'CIS-1.9': 'Password policies should require at least one uppercase letter',
        'CIS-1.10': 'Password policies should require at least one lowercase letter',
        'CIS-1.11': 'Password policies should require at least one symbol character',
        'CIS-1.12': 'Password policies should require at least one number',
        'CIS-1.13': 'Password minimum length provides primary defense against brute-force attacks',
        'CIS-1.14': 'Password reuse prevention stops users from cycling through old passwords',
        'CIS-1.15': 'Access keys should be regularly rotated to limit exposure window',
        'CIS-1.16': 'IAM policies should not grant full administrative privileges to users',
        'CIS-1.17': 'Support role should be set up to manage AWS support cases',
        'CIS-1.18': 'IAM instance roles should be used instead of access keys on EC2',
        'CIS-1.19': 'Expired SSL/TLS certificates should be removed from IAM',
        'CIS-1.20': 'Access Analyzer helps identify unintended resource access',
        'CIS-1.21': 'Centralized IAM user management improves security governance',
        'CIS-1.22': 'IAM user credentials should not be used for more than 45 days',
        
        # Section 2: Storage
        'CIS-2.1.1': 'S3 bucket encryption should be enabled by default to protect data at rest',
        'CIS-2.1.2': 'S3 buckets should restrict public read access to prevent data exposure',
        'CIS-2.1.3': 'S3 buckets should restrict public write access to prevent unauthorized uploads',
        'CIS-2.1.4': 'S3 bucket policy should restrict public access at the bucket level',
        'CIS-2.1.5': 'S3 Block Public Access should be enabled to prevent accidental exposure',
        'CIS-2.2.1': 'EBS default encryption ensures all volumes are encrypted automatically',
        'CIS-2.3.1': 'RDS instances should restrict public access to protect databases',
        'CIS-2.3.2': 'RDS snapshots should not be publicly accessible',
        'CIS-2.3.3': 'RDS encryption at rest protects database contents from unauthorized access',
        
        # Section 3: Logging
        'CIS-3.1': 'CloudTrail should be enabled in all regions for comprehensive audit logging',
        'CIS-3.2': 'CloudTrail log file validation ensures logs have not been tampered with',
        'CIS-3.3': 'S3 bucket used for CloudTrail logs should not be publicly accessible',
        'CIS-3.4': 'CloudTrail logs should be integrated with CloudWatch for real-time monitoring',
        'CIS-3.5': 'AWS Config should record all resource configuration changes',
        'CIS-3.6': 'S3 bucket access logging provides audit trail of bucket access',
        'CIS-3.7': 'CloudTrail logs should be encrypted at rest using KMS',
        'CIS-3.8': 'KMS key rotation ensures encryption keys are regularly changed',
        'CIS-3.9': 'VPC Flow Logs should be enabled to monitor network traffic',
        'CIS-3.10': 'Object-level logging in S3 provides detailed access records',
        'CIS-3.11': 'Object-level logging for write events tracks data modifications',
        
        # Section 4: Monitoring
        'CIS-4.1': 'Monitor unauthorized API calls to detect potential security breaches',
        'CIS-4.2': 'Monitor console sign-in without MFA to enforce MFA usage',
        'CIS-4.3': 'Monitor usage of root account to ensure it is rarely used',
        'CIS-4.4': 'Monitor IAM policy changes to detect unauthorized privilege escalation',
        'CIS-4.5': 'Monitor CloudTrail configuration changes to prevent audit log tampering',
        'CIS-4.6': 'Monitor failed console authentication to detect brute-force attacks',
        'CIS-4.7': 'Monitor customer-managed key deletion to prevent data loss',
        'CIS-4.8': 'Monitor S3 bucket policy changes to detect unauthorized access grants',
        'CIS-4.9': 'Monitor AWS Config configuration changes to maintain compliance',
        'CIS-4.10': 'Monitor security group changes to detect firewall rule modifications',
        'CIS-4.11': 'Monitor network ACL changes to detect network security changes',
        'CIS-4.12': 'Monitor network gateway changes to detect routing modifications',
        'CIS-4.13': 'Monitor route table changes to detect network path alterations',
        'CIS-4.14': 'Monitor VPC changes to detect virtual network modifications',
        'CIS-4.15': 'Monitor organizations changes to detect account structure modifications',
    }
    return descriptions.get(benchmark_id, '')
...
def generate_cis_benchmark_section(cis_data):
    ...
    html = '<div class="section">\n'
    html += '<h2>📋 CIS AWS Foundations Benchmark v1.4.0</h2>\n'
    ...
    not_verified = [c for c in checks if c.get('Status') == 'NOT_VERIFIED']
    
    # Count failures by severity
    critical = [c for c in failed if c.get('Severity') == 'CRITICAL']
    high = [c for c in failed if c.get('Severity') == 'HIGH']
    medium = [c for c in failed if c.get('Severity') == 'MEDIUM']
    
    # Summary banner
    total = len(checks)
    html += f'<div class="alert alert-info" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none;">\n'
    html += f'<h3 style="margin: 0 0 10px 0; color: white;">CIS Compliance Overview - {total} Total Controls</h3>\n'
    html += f'<div style="display: flex; gap: 20px; flex-wrap: wrap;">\n'
    html += f'<div style="flex: 1; min-width: 150px;"><strong>✅ Passed:</strong> {len(passed)} ({len(passed)*100//total if total > 0 else 0}%)</div>\n'
    html += f'<div style="flex: 1; min-width: 150px;"><strong>❌ Failed:</strong> {len(failed)} ({len(failed)*100//total if total > 0 else 0}%)</div>\n'
    html += f'<div style="flex: 1; min-width: 150px;"><strong>⚪ Not Verified:</strong> {len(not_verified)} ({len(not_verified)*100//total if total > 0 else 0}%)</div>\n'
    html += f'</div>\n'
    html += f'</div>\n'
    
    # Alert banners for failures
    if len(critical) > 0:
        html += f'<div class="alert alert-danger">🚨 {len(critical)} CRITICAL CIS compliance failure(s) - immediate action required!</div>\n'
    if len(high) > 0:
        html += f'<div class="alert alert-warning">⚠️ {len(high)} HIGH priority CIS compliance issue(s)</div>\n'
    if len(medium) > 0:
        html += f'<div class="alert alert-warning">⚠️ {len(medium)} MEDIUM priority CIS compliance issue(s)</div>\n'
    
    # Group checks by section
    section_1 = [c for c in checks if c.get('BenchmarkID', '').startswith('CIS-1')]
    section_2 = [c for c in checks if c.get('BenchmarkID', '').startswith('CIS-2')]
    section_3 = [c for c in checks if c.get('BenchmarkID', '').startswith('CIS-3')]
    section_4 = [c for c in checks if c.get('BenchmarkID', '').startswith('CIS-4')]
    
    sections = [
        ('Section 1: Identity and Access Management', section_1, 'iam'),
        ('Section 2: Storage', section_2, 'storage'),
        ('Section 3: Logging', section_3, 'logging'),
        ('Section 4: Monitoring', section_4, 'monitoring')
    ]
    
    for section_name, section_checks, section_id in sections:
        if not section_checks:
            continue
        
        section_passed = sum(1 for c in section_checks if c.get('Status') == 'PASS')
        section_failed = sum(1 for c in section_checks if c.get('Status') == 'FAIL')
        section_not_verified = sum(1 for c in section_checks if c.get('Status') == 'NOT_VERIFIED')
        section_total = len(section_checks)
        
        html += f'''
        <h3 style="margin-top: 30px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            onclick="toggleSection('cis-{section_id}')">
            <span id="cis-{section_id}-icon" style="display: inline-block; width: 20px;">&#9658;</span>
            {section_name}
            <span style="color: #666; font-size: 14px; font-weight: normal; margin-left: 10px;">
                ({section_total} controls: ✅ {section_passed} | ❌ {section_failed} | ⚪ {section_not_verified})
            </span>
        </h3>
        '''
        
        html += f'<div id="cis-{section_id}" style="display: none;">\n'
        html += '<table>\n'
        html += '<thead><tr><th>ID</th><th>Resource</th><th>Status</th><th>Severity</th><th>Finding</th><th>Recommendation</th></tr></thead>\n'
        html += '<tbody>\n'
        ...
        for check in section_checks:
            ...
            description = get_cis_control_description(benchmark_id)
            description_html = ''
            if description:
                description_html = f'<div style="color: #666; font-size: 11px; font-style: italic; margin-top: 3px;">{description}</div>'
            
            html += f'''<tr>
                <td><span class="code">{benchmark_id}</span>{description_html}</td>
                <td>{resource}</td>
                <td><span class="badge {status_class}">{status}</span></td>
                <td><span class="badge {severity_badge}">{severity}</span></td>
                <td><small>{finding}</small></td>
                <td><small>{recommendation}</small></td>
            </tr>\n'''
...
def generate_html_report(verification_data):
    ...
    for section in verification_data.get('Sections', []):
        ...
        if 'VPC' in title:
            ...
            content += generate_load_balancer_section(section)
...
def main():
    ...
    # Save HTML with UTF-8 encoding (fixes Windows cp1252 encoding error)
