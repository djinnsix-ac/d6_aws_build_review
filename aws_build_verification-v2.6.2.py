...
aws s3api put-bucket-logging --bucket {bucket_name} --bucket-logging-status '{{"LoggingEnabled": {{"TargetBucket": "YOUR-LOGGING-BUCKET", "TargetPrefix": "{bucket_name}/"}}}}' ''',
  ...
  target_prefix = "{bucket_name}/"
    ...
    def verify_cis_benchmarks(self) -> Dict[str, Any]:
        """Verify CIS AWS Foundations Benchmark - Phase 1 Critical Checks"""
        results = {
        ...
        if 'Error' in security_audit or not security_audit:
            results['checks'].append({
                'BenchmarkID': 'CIS-AUDIT',
                'Resource': 'Security Audit',
                'Status': 'Error',
                'Severity': 'INFO',
                'Details': 'Enable SecurityAudit permissions for CIS checks'
            })
            return results
        
        # CIS 1.4: Root account access keys
        root_checks = security_audit.get('RootAccountChecks', {})
        if root_checks.get('AccessKey1Active') or root_checks.get('AccessKey2Active'):
            results['checks'].append({
                'BenchmarkID': 'CIS-1.4',
        ...
        # Root MFA - Enhanced check with Organizations/SSO context
        virtual_mfa_active = root_checks.get('MFAActive', False)
        hardware_mfa_devices = root_checks.get('HardwareMFADevices', [])
        has_any_mfa = virtual_mfa_active or len(hardware_mfa_devices) > 0
        
        # Check Organizations and SSO context
        org_data = self.collected_data.get('Organizations', {})
        is_member_account = org_data.get('IsMemberAccount', False)
        sso_enabled = org_data.get('SSOEnabled', False)
        
        if not has_any_mfa:
            # Determine severity based on context
            if is_member_account and sso_enabled:
                # Lower severity if managed by Organizations + SSO
                severity = 'MEDIUM'
                finding = 'Root account MFA not detected via IAM credential report'
                recommendation = '⚠️ MEDIUM: Virtual/hardware MFA not detected in IAM. If root access is managed centrally via AWS Organizations/SSO, this may be expected. Otherwise, enable MFA for root account.'
            elif is_member_account:
                # Member account but no SSO detected
                severity = 'HIGH'
                finding = 'Root account MFA not detected (member account)'
                recommendation = '⚠️ HIGH: Virtual/hardware MFA not detected. This is a member account in an AWS Organization. Verify if root MFA is managed centrally or enable MFA.'
            else:
                # Standalone account - critical
                severity = 'CRITICAL'
                finding = 'Root account MFA not enabled'
                recommendation = '🔒 CRITICAL: Enable MFA for root account immediately'
        ...
        if not pw_policy.get('Exists'):
            results['checks'].append({
                'BenchmarkID': 'CIS-1.5-1.11',
        ...
        if not any(t.get('IsMultiRegionTrail') and t.get('IsLogging') for t in trails):
            results['checks'].append({
                'BenchmarkID': 'CIS-3.1',
                'Resource': 'CloudTrail',
                'Status': 'FAIL',
                'Severity': 'CRITICAL',
                'Finding': 'No multi-region CloudTrail logging',
                'Recommendation': '🔒 CRITICAL: Enable CloudTrail in all regions'
            })
        
        # VPC Flow Logs
        vpc_flow = security_audit.get('VPCFlowLogs', {})
        if vpc_flow.get('TotalVPCs', 0) > vpc_flow.get('VPCsWithFlowLogs', 0):
            results['checks'].append({
                'BenchmarkID': 'CIS-3.7',
                'Resource': 'VPC Flow Logs',
                'Status': 'FAIL',
                'Severity': 'MEDIUM',
                'Finding': f'{vpc_flow.get("TotalVPCs", 0) - vpc_flow.get("VPCsWithFlowLogs", 0)} VPC(s) without Flow Logs',
                'Recommendation': '🔒 MEDIUM: Enable VPC Flow Logs on all VPCs'
            })
        
        # AWS Config
        if not security_audit.get('AWSConfig', {}).get('Enabled'):
            results['checks'].append({
                'BenchmarkID': 'CIS-3.5',
                'Resource': 'AWS Config',
                'Status': 'FAIL',
                ...
                'Recommendation': '🔒 MEDIUM: Enable AWS Config'
            })
        
        # Summary if all pass
        if not results['checks']:
            results['checks'].append({
                'BenchmarkID': 'CIS-SUMMARY',
                'Resource': 'CIS Benchmark',
                'Status': 'PASS',
                'Severity': 'INFO',
                'Details': 'Core CIS checks passed'
            })
