...
<head>
    ...
    <style>
        * {{
        ...
        /* Inner table for detailed security group rules */
        .inner-table {{
            width: 100%;
            border-collapse: collapse;
            margin: 10px 0;
            background: white;
            font-size: 12px;  /* Uniform font size for all content */
        }}
        ...
        .inner-table th {{
            ...
            text-align: left;
            font-weight: 600;
            font-size: 12px;  /* Smaller header font for inner table */
        ...
        .inner-table th:nth-child(1),  /* Rule # */
        .inner-table td:nth-child(1) {{
            width: 5%;
            min-width: 40px;
        }}
        
        .inner-table th:nth-child(2),  /* Protocol */
        .inner-table td:nth-child(2) {{
            width: 8%;
            min-width: 60px;
        }}
        
        .inner-table th:nth-child(3),  /* Port */
        .inner-table td:nth-child(3) {{
            width: 8%;
            min-width: 60px;
        }}
        
        .inner-table th:nth-child(4),  /* Source */
        .inner-table td:nth-child(4) {{
            width: 15%;
            min-width: 120px;
        }}
        
        .inner-table th:nth-child(5),  /* Description */
        .inner-table td:nth-child(5) {{
            width: 20%;
            min-width: 150px;
        }}
        
        .inner-table th:nth-child(6),  /* Risk Assessment */
        .inner-table td:nth-child(6) {{
            width: 34%;
            min-width: 250px;
        }}
        
        .inner-table th:nth-child(7),  /* Status */
        .inner-table td:nth-child(7) {{
            width: 10%;
            min-width: 70px;
            text-align: center;
        }}
        
        /* Ensure small tags don't change font size */
        .inner-table small {{
            font-size: 11px;
        }}
        
        /* Badge sizing in inner table */
        .inner-table .badge {{
            font-size: 10px;
            padding: 3px 6px;
        }}
...
'''
    ...
    for idx, sg in enumerate(sg_data.get('checks', [])):
        ...
        if ingress_count > 0:
            ...
            # Check if we have the new detailed analysis
            detailed_rules = sg.get('DetailedIngressRules', [])
            
            if detailed_rules:
                # NEW: Display per-rule analysis with severity badges
                ingress_rules_html += '<table class="inner-table"><thead><tr>'
                ingress_rules_html += '<th>Rule</th><th>Protocol</th><th>Port</th><th>Source</th>'
                ingress_rules_html += '<th>Description</th><th>Risk Assessment</th><th>Status</th>'
                ingress_rules_html += '</tr></thead><tbody>'
                
                for rule in detailed_rules:
                    rule_severity = rule.get('Severity', 'INFO')
                    rule_badge = f'badge-{rule_severity.lower()}'
                    
                    # Format port display
                    from_port = rule.get('FromPort', 'All')
                    to_port = rule.get('ToPort', 'All')
                    port_display = str(from_port) if from_port == to_port else f"{from_port}-{to_port}"
                    
                    # Format source with badge
                    source = rule.get('Source', '')
                    source_badge = ''
                    if rule.get('IsOpenToInternet'):
                        source_badge = ' <span class="badge badge-warning">PUBLIC</span>'
                    elif rule.get('SourceType') == 'SecurityGroup':
                        source_badge = ' <span class="badge badge-info">SG</span>'
                    
                    if rule.get('IPVersion') == 'IPv6':
                        source += ' <small>(IPv6)</small>'
                    ...
                    ingress_rules_html += f'''<tr>
                        <td>#{rule.get("RuleNumber", "")}</td>
                        <td>{rule.get("Protocol", "")}</td>
                        <td>{port_display}</td>
                        <td><code>{source}</code>{source_badge}</td>
                        <td>{description}</td>
                        <td>{rule.get("RiskReason", "")}</td>
                        <td><span class="badge {rule_badge}">{rule_severity}</span></td>
                    </tr>'''
                
                ingress_rules_html += '</tbody></table>'
                
            else:
                # Fall back to old format if detailed rules not available
                ...
                if ingress_details:
                    ...
                    ingress_rules_html = '''<div class="alert alert-info">
                        <strong>Note:</strong> Detailed rule analysis requires updated verification script (v2.5.6+). 
