...
Changelog:
- v2.4.2: Fixed CloudTrail collection for Control Tower/Organization trails
  - Organization trails visible via describe_trails but status not queryable from member accounts
  - Now includes trail even if get_trail_status fails, marks as org-managed
- v2.4.1: Minor fixes
- v2.4.0: FEATURE - Enhanced IAM policy collection with full policy document retrieval
- v2.3.4: Added DynamoDB table collection with encryption and PITR details
- v2.3.3: Added OpenSearch domain collection
- v2.3.2: BUGFIX - VPC Peering now collects connections where VPC is accepter (not just requester)
...
class AWSBuildReviewer:
    ...
    def get_vpc_configuration(self) -> Dict[str, Any]:
        ...
        for vpc in vpcs:
            ...
            # Get VPC Peering Connections (both as requester and accepter)
            ...
            # Combine and deduplicate (in case of same-account peering)
            peering_ids = set()
            peering = []
    ...
    def get_iam_configuration(self) -> Dict[str, Any]:
        ...
        # Get customer-managed policies with full policy documents
        ...
        for policy in policies:
            ...
            try:
                ...
                # If we can't get the policy document, still include the metadata
                policy_details.append({
                    'PolicyMetadata': policy,
                    'PolicyDocument': None,
                    'Error': str(e)
        ...
        return {
            'Roles': role_data,
            'Users': users,
            'CustomerManagedPolicies': policy_details  # Now includes full policy documents
        }
    
    def get_organizations_info(self) -> Dict[str, Any]:
        """Detect AWS Organizations membership and SSO configuration"""
        data = {
        ...
        try:
            ...
            data['IsMemberAccount'] = True
            data['OrganizationId'] = org_info['Organization']['Id']
            data['MasterAccountId'] = org_info['Organization']['MasterAccountId']
            data['OrganizationArn'] = org_info['Organization']['Arn']
        except org_client.exceptions.AWSOrganizationsNotInUseException:
            # Account is not part of an organization
            data['IsMemberAccount'] = False
        except Exception as e:
            # Might not have permissions or other error
            data['Errors'].append(f"Organizations check: {str(e)}")
        
        # Check for AWS SSO/Identity Center
        try:
            sso_admin = self.session.client('sso-admin')
            sso_instances = sso_admin.list_instances()
            if sso_instances.get('Instances'):
                data['SSOEnabled'] = True
                data['SSOInstances'] = sso_instances['Instances']
        except Exception as e:
            data['Errors'].append(f"SSO check: {str(e)}")
        
        # Check for Identity Center (newer name for SSO)
        try:
            identity_store = self.session.client('identitystore')
            # If we can call this, Identity Center is likely configured
            # But we need an instance ARN to do more, which we got above
            if data['SSOInstances']:
                data['IdentityCenterConfigured'] = True
        except Exception as e:
            data['Errors'].append(f"Identity Center check: {str(e)}")
    ...
    def get_security_audit_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                for trail in trails:
                    ...
                    trail_info = {
                        ...
                        'IsOrganizationTrail': is_org_trail
                    }
                    ...
                    try:
                        ...
                        trail_info['EventSelectors'] = event_selectors
                        
                    except Exception as e:
                        # Organization trails may not be queryable from member accounts
                        # but we still know they exist and are likely logging
                        if is_org_trail or 'controltower' in trail_name.lower():
                            trail_info['Status'] = {'Note': 'Organization/Control Tower trail - status managed centrally'}
                            trail_info['IsLogging'] = True  # Assume org trails are logging
                            trail_info['EventSelectors'] = {}
                            data['Errors'].append(f"Org trail {trail_name}: Status not queryable from member account (expected)")
                        else:
                            data['Errors'].append(f"GetTrailStatus/EventSelectors {trail_name}: {str(e)}")
                            trail_info['Status'] = {'Error': str(e)}
                            trail_info['IsLogging'] = False
            ...
            try:
                ...
                # Check for hardware MFA devices on root account
                try:
                    # List all MFA devices (includes hardware tokens)
                    mfa_devices = iam.list_mfa_devices()
                    data['IAMCredentialReport']['AllMFADevices'] = mfa_devices.get('MFADevices', [])
                    
                    # Check specifically for root account MFA
                    root_mfa = [d for d in mfa_devices.get('MFADevices', []) if d.get('UserName') == 'root']
                    data['RootAccountChecks']['HardwareMFADevices'] = root_mfa
                except Exception as e:
                    data['Errors'].append(f"List MFA Devices: {str(e)}")
    ...
    def get_opensearch_configuration(self) -> Dict[str, Any]:
        """Collect OpenSearch domain configurations"""
        try:
            ...
            try:
                domain_names_response = opensearch.list_domain_names()
                domain_names = [d['DomainName'] for d in domain_names_response.get('DomainNames', [])]
                
                # Get detailed configuration for each domain
                for domain_name in domain_names:
                    try:
                        domain_status = opensearch.describe_domain(DomainName=domain_name)
                        domain_detail = domain_status.get('DomainStatus', {})
                        
                        # Get domain config
                        try:
                            config_response = opensearch.describe_domain_config(DomainName=domain_name)
                            domain_detail['DomainConfig'] = config_response.get('DomainConfig', {})
                        except Exception as e:
                            data['Errors'].append(f"DescribeDomainConfig {domain_name}: {str(e)}")
                        
                        # Get tags
                        try:
                            tags_response = opensearch.list_tags(ARN=domain_detail.get('ARN'))
                            domain_detail['Tags'] = tags_response.get('TagList', [])
                        except Exception as e:
                            data['Errors'].append(f"ListTags {domain_name}: {str(e)}")
                        
                        data['Domains'].append(domain_detail)
                    except Exception as e:
                        data['Errors'].append(f"DescribeDomain {domain_name}: {str(e)}")
            except Exception as e:
                data['Errors'].append(f"ListDomainNames: {str(e)}")
    ...
    def collect_all_data(self) -> Dict[str, Any]:
        ...
        collections = [
            ...
            ('Organizations', self.get_organizations_info),
            ...
            ('OpenSearch', self.get_opensearch_configuration),
        ]
