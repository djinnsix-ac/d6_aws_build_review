...
Changelog:
- v2.13.0: Added ECS, EKS, and Load Balancers to Compute section - display everything we collect
...
def generate_compute_section(compute_data):
    ...
    if compute_data.get('ecs'):
        ...
        for cluster in compute_data['ecs']:
            ...
            html += '<tbody>\n'
            html += f'<tr><td>ARN</td><td><span class="code">{cluster_arn}</span></td></tr>\n'
            html += f'<tr><td>Status</td><td>{cluster.get("Status", "")}</td></tr>\n'
            html += f'<tr><td>Registered Instances</td><td>{cluster.get("RegisteredContainerInstancesCount", 0)}</td></tr>\n'
            html += f'<tr><td>Running Tasks</td><td>{cluster.get("RunningTasksCount", 0)}</td></tr>\n'
            html += f'<tr><td>Pending Tasks</td><td>{cluster.get("PendingTasksCount", 0)}</td></tr>\n'
            html += f'<tr><td>Active Services</td><td>{cluster.get("ActiveServicesCount", 0)}</td></tr>\n'
            html += '</tbody></table>\n'
            ...
            if services:
                html += '<h5>Services</h5>\n'
                html += '<table>\n'
                html += '<thead><tr><th>Service Name</th><th>Status</th><th>Desired</th><th>Running</th><th>Pending</th><th>Launch Type</th></tr></thead>\n'
                html += '<tbody>\n'
                ...
                for service in services:
                    ...
                    html += f'''<tr>
                        ...
                        <td>{service.get("PendingCount", 0)}</td>
    ...
    if compute_data.get('eks'):
        ...
        for cluster in compute_data['eks']:
            ...
            html += f'''<tr>
                <td><span class="code">{cluster.get("Name", "")}</span></td>
                <td><span class="badge {badge}">{status}</span></td>
                <td>{cluster.get("Version", "")}</td>
                <td><span class="code">{cluster.get("Endpoint", "")}</span></td>
                <td>{cluster.get("PlatformVersion", "")}</td>
                <td><span class="code">{cluster.get("VpcId", "")}</span></td>
            </tr>\n'''
        
        html += '</tbody></table>\n'
    
    # Load Balancers (ALB/NLB)
    if compute_data.get('load_balancers'):
        html += '<h3>Load Balancers</h3>\n'
        html += '<table>\n'
        html += '<thead><tr><th>Name</th><th>Type</th><th>Scheme</th><th>State</th><th>VPC</th><th>AZs</th><th>DNS Name</th></tr></thead>\n'
        html += '<tbody>\n'
        
        for lb in compute_data['load_balancers']:
            state = lb.get('State', {}).get('Code', '')
            badge = 'badge-success' if state == 'active' else 'badge-warning'
            ...
            html += f'''<tr>
                ...
                <td><span class="badge {badge}">{state}</span></td>
                <td><span class="code">{lb.get("VpcId", "")}</span></td>
                <td>{az_count}</td>
                <td><span class="code">{lb.get("DNSName", "")}</span></td>
            </tr>\n'''
