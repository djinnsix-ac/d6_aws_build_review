...
def generate_storage_section(storage_data):
    ...
    html += '<table>\n'
    html += '<thead><tr><th>Bucket Name</th><th>Region</th><th>Versioning</th><th>Encryption</th><th>Public Block</th><th>Logging</th><th>Security Score</th></tr></thead>\n'
    html += '<tbody>\n'
    
    for bucket in storage_data['checks']:
        score = bucket.get('SecurityScore', 0)
        score_badge = 'badge-success' if score >= 3 else 'badge-warning' if score >= 2 else 'badge-danger'
        
        html += f'''<tr>
            <td><span class="code">{bucket.get("BucketName", "")}</span></td>
            <td>{bucket.get("Region", "N/A")}</td>
            <td>{bucket.get("Versioning", "Disabled")}</td>
            <td>{bucket.get("Encryption", "Disabled")}</td>
            <td>{bucket.get("PublicAccessBlock", "Not Configured")}</td>
            <td>{bucket.get("Logging", "Disabled")}</td>
            <td><span class="badge {score_badge}">{score}/4</span></td>
        </tr>\n'''
