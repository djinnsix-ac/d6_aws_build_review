...
def generate_database_section(db_data):
    ...
    if db_data.get('rds'):
        ...
        for db in db_data['rds']:
            ...
            if db.get('KmsKeyId'):
                html += f'<tr><td><strong>KMS Key</strong></td><td><span class="code" style="font-size: 10px;">{db.get("KmsKeyId")}</span></td></tr>\n'
            ...
            if security_groups:
                sg_list = '<br>'.join(security_groups)
    ...
    if db_data.get('opensearch'):
        ...
        for domain in db_data['opensearch']:
            ...
            html += f'''<h5 style="margin-top: 20px; color: #2c5aa0; font-size: 15px; cursor: pointer; padding: 8px 0;" 
                onclick="toggleSection('os-{domain_name}')">
                <span id="os-{domain_name}-icon" style="display: inline-block; width: 20px;">&#9658;</span>
                {domain_name}
            </h5>\n'''
            
            html += f'<div id="os-{domain_name}" style="display: none;">\n'
            html += '<table>\n'
            html += '<thead><tr><th style="width: 30%;">Property</th><th>Value</th></tr></thead>\n'
            html += '<tbody>\n'
            ...
            html += f'<tr><td><strong>Instance Count</strong></td><td>{domain.get("InstanceCount", 0)}</td></tr>\n'
            ...
            zone_aware = '✅ Yes' if domain.get('ZoneAwarenessEnabled') else '❌ No'
            html += f'<tr><td><strong>Zone Awareness</strong></td><td>{zone_aware}</td></tr>\n'
            
            # Security
            encrypt_rest = '✅ Enabled' if domain.get('EncryptionAtRestEnabled') else '❌ Disabled'
            html += f'<tr><td><strong>Encryption at Rest</strong></td><td>{encrypt_rest}</td></tr>\n'
            
            encrypt_node = '✅ Enabled' if domain.get('NodeToNodeEncryptionEnabled') else '❌ Disabled'
            html += f'<tr><td><strong>Node-to-Node Encryption</strong></td><td>{encrypt_node}</td></tr>\n'
