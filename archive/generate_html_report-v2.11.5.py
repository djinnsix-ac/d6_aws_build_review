...
def format_security_group_rule(rule, rule_type='ingress', idx=0):
    ...
    for ip_range in rule.get('IpRanges', []):
        ...
        <div style="padding: 10px; margin: 5px 0; background: #f9f9f9; border-radius: 4px; border-left: 3px solid {"#dc3545" if not compliance["is_compliant"] else "#28a745"};">
            ...
            {issues_html if rule_counter == 1 else ''}
        </div>
        ''')
        rule_counter += 1
    ...
    for sg_ref in rule.get('UserIdGroupPairs', []):
        ...
        <div style="padding: 10px; margin: 5px 0; background: #f9f9f9; border-radius: 4px; border-left: 3px solid {"#dc3545" if not compliance["is_compliant"] else "#28a745"};">
            ...
            {issues_html if rule_counter == 1 and not rule.get('IpRanges') else ''}
        </div>
        ''')
        rule_counter += 1
    ...
    for ipv6_range in rule.get('Ipv6Ranges', []):
        ...
        <div style="padding: 10px; margin: 5px 0; background: #f9f9f9; border-radius: 4px; border-left: 3px solid {"#dc3545" if not compliance["is_compliant"] else "#28a745"};">
            ...
            {issues_html if rule_counter == 1 and not rule.get('IpRanges') and not rule.get('UserIdGroupPairs') else ''}
        </div>
        ''')
        rule_counter += 1
    ...
    for prefix_list in rule.get('PrefixListIds', []):
        ...
        <div style="padding: 10px; margin: 5px 0; background: #f9f9f9; border-radius: 4px; border-left: 3px solid {"#dc3545" if not compliance["is_compliant"] else "#28a745"};">
            ...
            {issues_html if rule_counter == 1 and not rule.get('IpRanges') and not rule.get('UserIdGroupPairs') and not rule.get('Ipv6Ranges') else ''}
        </div>
        ''')
        rule_counter += 1
