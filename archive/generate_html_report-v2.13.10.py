...
def generate_database_section(db_data):
    ...
    if db_data.get('rds'):
        ...
        for db in db_data['rds']:
            ...
            if subnets:
                # Remove leading spaces and sort subnets
                subnet_list = '<br>'.join([s.strip() for s in sorted(subnets)])
                html += f'<tr><td><strong>Subnets</strong></td><td><span class="code">{subnet_list}</span></td></tr>\n'
            
            security_groups = db.get('SecurityGroups', [])
            if security_groups:
                # Remove leading spaces and sort security groups
                sg_list = '<br>'.join([sg.strip() for sg in sorted(security_groups)])
                html += f'<tr><td><strong>Security Groups</strong></td><td><span class="code">{sg_list}</span></td></tr>\n'
    ...
    if db_data.get('opensearch'):
        ...
        for domain in db_data['opensearch']:
            ...
            # Access Policies - clickable like target groups
            ...
            if access_policies:
                html += f'''<tr><td><strong>Access Policies</strong></td><td>
                    ...
                    <div id="os-{domain_id_safe}-policies" style="display: none; margin-top: 8px; padding: 10px; background: #f5f5f5; border-radius: 4px; font-family: monospace; font-size: 11px; white-space: pre-wrap; word-break: break-all;">
{access_policies}
                    </div>
                </td></tr>\n'''
