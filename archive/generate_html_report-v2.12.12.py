...
def generate_vpc_section(vpc_data, collected_data=None):
    ...
    # Network ACLs Section (h4 collapsible) - Show ALL NACLs across ALL VPCs
    all_network_acls = []
    all_subnets = []
    for vpc in raw_vpcs:
        nacls = vpc.get('NetworkACLs', [])
        all_network_acls.extend(nacls)
        all_subnets.extend(vpc.get('Subnets', []))
    
    if all_network_acls:
        nacl_section_id = "network-acls-all"
        html += f'''<h4 style="margin-top: 20px; color: #667eea; font-size: 18px; font-weight: 600; cursor: pointer; padding: 8px 0; border-bottom: 3px solid #667eea;" 
            ...
            Network ACLs ({len(all_network_acls)})
        </h4>\n'''
        
        html += f'<div id="{nacl_section_id}" style="display: none;">\n'
        html += '<table>\n'
        html += '<thead><tr>'
        html += '<th style="width: 18%;">NACL ID</th>'
        html += '<th style="width: 20%;">VPC ID</th>'
        html += '<th style="width: 12%;">Owner ID</th>'
        html += '<th style="width: 8%;">Default</th>'
        html += '<th style="width: 8%;">Inbound</th>'
        html += '<th style="width: 8%;">Outbound</th>'
        html += '<th style="width: 26%;">Associated Subnets</th>'
        html += '</tr></thead>\n'
        html += '<tbody>\n'
        
        for idx, nacl in enumerate(all_network_acls):
            nacl_id = nacl.get('NetworkAclId', 'N/A')
            nacl_vpc_id = nacl.get('VpcId', 'N/A')
            ...
            for assoc_idx, assoc in enumerate(associations):
                ...
                if subnet_id:
                    # Find full subnet data
                    full_subnet = next((s for s in all_subnets if s.get('SubnetId') == subnet_id), None)
                    if full_subnet:
                        # Make subnet clickable
                        subnet_link = f'<a href="#" onclick="toggleRules(\'nacl-subnet-all-{idx}-{assoc_idx}\'); return false;" style="color: #059669; text-decoration: underline; cursor: pointer;">{subnet_id}</a>'
                        subnet_list.append(subnet_link)
                        # Store for details row
                        assoc['_full_subnet'] = full_subnet
                        assoc['_detail_id'] = f'nacl-subnet-all-{idx}-{assoc_idx}'
                    else:
            ...
            nacl_link = f'<a href="#" onclick="toggleRules(\'nacl-details-all-{idx}\'); return false;" style="color: #2563eb; text-decoration: underline; cursor: pointer;">{nacl_id}</a>'
            ...
            html += f'''<tr>
                ...
                <td><span class="code">{nacl_vpc_id}</span></td>
                <td>{owner_id}</td>
            ...
            html += f'''<tr id="nacl-details-all-{idx}" style="display:none;">
                <td colspan="7">{generate_nacl_details(nacl)}</td>
            </tr>\n'''
            ...
            for assoc in associations:
                if '_full_subnet' in assoc and '_detail_id' in assoc:
                    ...
                    full_subnet = assoc['_full_subnet']
                    html += f'''<tr id="{detail_id}" style="display:none;">
                        <td colspan="7">{generate_subnet_details(full_subnet)}</td>
                    </tr>\n'''
