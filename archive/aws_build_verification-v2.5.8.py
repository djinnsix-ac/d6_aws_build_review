...
class BuildVerifier:
    ...
    def verify_dynamodb(self) -> Dict[str, Any]:
        ...
        for table in tables:
            ...
            encryption_type = sse.get('SSEType', 'None') if encryption_enabled else 'Disabled'
            ...
            if not encryption_enabled:
                issues.append('Encryption at rest disabled - data not protected')
                severity = 'HIGH'
            elif encryption_type != 'KMS':
                issues.append('Using AWS managed encryption instead of customer managed KMS')
                if severity == 'INFO':
                    severity = 'MEDIUM'
