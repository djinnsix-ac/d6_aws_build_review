...
def format_security_group_rule(rule, rule_type='ingress', idx=0):
    ...
    for prefix_list in rule.get('PrefixListIds', []):
        ...
        targets.append(f"pl: {pl_id}" + (f" ({desc})" if desc else " (VPC Endpoint)"))
...
<script>
    ...
    function initSecurityGroupGraph() {
        ...
        const prefixLists = new Set();
        links.forEach(l => {
            if (l.type === 'prefix-list') {
                prefixLists.add(l.target);
            }
        });
        
        prefixLists.forEach(plId => {
            nodes.push({
                id: plId,
                name: plId,
                isPrefixList: true,
        ...
        if (prefixLists.size > 0) {
            console.log('Added prefix list nodes:', prefixLists.size);
        }
