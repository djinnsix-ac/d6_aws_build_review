...
def generate_vpc_section(vpc_data, collected_data=None):
    ...
    for vpc_id, checks in vpc_checks.items():
        html += f'<h3>VPC: <span class="code">{vpc_id}</span></h3>\n'
        
        # Find raw VPC data for this VPC
        raw_vpc = next((v for v in raw_vpcs if v.get('VpcId') == vpc_id), None)
        
        # Display basic VPC checks
        html += '<h4>Configuration</h4>\n'
        html += '<table>\n<thead><tr><th>Check</th><th>Details</th></tr></thead>\n<tbody>\n'
        
        for check in checks:
            check_name = check.get('Check', 'Unknown')
            # Skip checks we'll show elsewhere
            if check_name in ['VPC Configuration', 'Subnet Distribution', 'Internet Gateway', 'NAT Gateways']:
                # Display these as summary rows
                details = []
                for key, value in check.items():
                    if key not in ['VPC', 'Check']:
                        if isinstance(value, dict):
                            details.append(f"{key}: {json.dumps(value)}")
                        elif isinstance(value, bool):
                            details.append(f"{key}: {'✓' if value else '✗'}")
                        else:
                            details.append(f"{key}: {value}")
                
                html += f'<tr><td><strong>{check_name}</strong></td><td>{", ".join(details) if details else "N/A"}</td></tr>\n'
        ...
        if raw_vpc:
            ...
            if vpc_endpoints:
                html += f'<h4>VPC Endpoints ({len(vpc_endpoints)})</h4>\n'
                html += '<table>\n'
        ...
        if raw_vpc:
            ...
            if peering_connections:
                html += f'<h4>VPC Peering Connections ({len(peering_connections)})</h4>\n'
                html += '<table>\n'
