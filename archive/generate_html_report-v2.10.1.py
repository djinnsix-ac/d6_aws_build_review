...
<script>
    ...
    function initSecurityGroupGraph() {
        ...
        // Add arrow marker for egress links (blue, smaller)
        svg.append('defs').append('marker')
            .attr('id', 'arrowhead')
            .attr('viewBox', '-0 -3 6 6')
            .attr('refX', 18)  // Position closer to target node
            .attr('refY', 0)
            .attr('orient', 'auto')
            .attr('markerWidth', 5)
            .attr('markerHeight', 5)
            ...
            .attr('fill', '#4e8ac9');  // Blue to distinguish from green ingress
        ...
        const link = g.append('g')
            ...
            .attr('marker-end', d => d.direction === 'egress' ? 'url(#arrowhead)' : '')
        ...
        node.append('circle')
            .attr('r', d => {
                if (d.isSpecial) return 25;
                // Size based on total rules (10-30 range)
                return Math.min(30, 10 + d.totalRules);
            })
            .attr('fill', d => {
                if (d.isSpecial) return '#dc3545';
                if (d.hasInternetAccess) return '#ff6b6b';
                return '#4e8ac9';
            })
            .attr('stroke', d => d.hasPermissiveRules ? '#ff0000' : '#fff')
            .attr('stroke-width', d => d.hasPermissiveRules ? 3 : 2);
        ...
        window.resetGraph = function() {
            ...
            link.style('display', d => {
                if (!document.getElementById('showEgress').checked && d.direction === 'egress') {
                    return 'none';
                }
                return null;
            });
