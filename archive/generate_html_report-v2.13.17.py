...
def generate_dynamodb_section(dynamodb_data):
    """Generate HTML for DynamoDB tables section"""
    if not dynamodb_data or 'checks' not in dynamodb_data:
        return ""
    
    checks = dynamodb_data.get('checks', [])
    if not checks:
        return ""
    
    html = '<div class="section">\n'
    html += '<h2>DynamoDB Tables</h2>\n'
    
    # Count issues by severity
    high_risk = sum(1 for table in checks if table.get('Severity') == 'HIGH')
    medium_risk = sum(1 for table in checks if table.get('Severity') == 'MEDIUM')
    low_risk = sum(1 for table in checks if table.get('Severity') == 'LOW')
    
    # Alert banner
    if high_risk > 0:
        html += f'<div class="alert alert-danger">⚠️ Found {high_risk} table(s) with HIGH severity security issues</div>\n'
    elif medium_risk > 0:
        html += f'<div class="alert alert-warning">⚠️ Found {medium_risk} table(s) with MEDIUM severity issues</div>\n'
    elif low_risk > 0:
        html += f'<div class="alert alert-info">ℹ️ Found {low_risk} table(s) with LOW severity recommendations</div>\n'
    else:
        html += '<div class="alert alert-success">✅ All DynamoDB tables follow security best practices</div>\n'
    
    # Summary statistics
    total_items = sum(table.get('ItemCount', 0) for table in checks)
    total_size_bytes = sum(table.get('TableSize', 0) for table in checks)
    ...
    html += f'<p><strong>Summary:</strong> {len(checks)} table(s) | {total_items:,} total items | {total_size} total size</p>\n'
    ...
    html += '<th>Status</th>'
    html += '<th>Billing Mode</th>'
    html += '<th>Encryption</th>'
    html += '<th>PITR</th>'
    html += '<th>Deletion Protection</th>'
    html += '<th>Items</th>'
    html += '<th>Size</th>'
    html += '<th>Severity</th>'
    html += '</tr></thead>\n'
    ...
    for idx, table in enumerate(checks):
        ...
        item_count = f"{table.get('ItemCount', 0):,}"
        size_display = table.get('TableSizeDisplay', '0 bytes')
        
        # Status colors
        status_class = 'text-success' if status == 'ACTIVE' else 'text-warning'
        encryption_class = 'text-success' if table.get('EncryptionEnabled') else 'text-danger'
        pitr_class = 'text-success' if table.get('PITREnabled') else 'text-warning'
        deletion_class = 'text-success' if table.get('DeletionProtected') else 'text-warning'
        
        # Main row
        row_class = 'issue-row' if table.get('Issues') else ''
        html += f'<tr class="{row_class}">\n'
        html += f'  <td><strong>{table_name}</strong></td>\n'
        html += f'  <td><span class="{status_class}">{status}</span></td>\n'
        html += f'  <td>{billing}</td>\n'
        html += f'  <td><span class="{encryption_class}">{encryption}</span></td>\n'
        html += f'  <td><span class="{pitr_class}">{pitr}</span></td>\n'
        html += f'  <td><span class="{deletion_class}">{deletion_protection}</span></td>\n'
        html += f'  <td style="text-align: right;">{item_count}</td>\n'
        html += f'  <td style="text-align: right;">{size_display}</td>\n'
        html += f'  <td><span class="badge {badge_class}">{severity}</span></td>\n'
        html += '</tr>\n'
        ...
        if table.get('HasTags'):
            html += '<tr class="tags-row">\n'
            html += '  <td colspan="9">\n'
            html += '    <div style="padding: 10px;">\n'
            html += '      <strong>Tags:</strong> '
            tags_display = ', '.join([f"{k}={v}" for k, v in table.get('Tags', {}).items()])
            html += f'{tags_display}\n'
            html += '    </div>\n'
            html += '  </td>\n'
            html += '</tr>\n'
    ...
    if high_risk > 0 or medium_risk > 0 or low_risk > 0:
        html += '<div style="margin-top: 20px; padding: 15px; background: #e7f3ff; border-left: 4px solid #2196F3;">\n'
        html += '<h4>💡 Recommendations</h4>\n'
        html += '<ul>\n'
        if high_risk > 0:
            html += '<li><strong>HIGH Priority:</strong> Address critical security issues immediately</li>\n'
        if medium_risk > 0:
            html += '<li><strong>MEDIUM Priority:</strong> Enable point-in-time recovery (PITR) for production tables to enable point-in-time restores</li>\n'
        if low_risk > 0:
            html += '<li><strong>LOW Priority:</strong> Consider using customer-managed KMS keys for additional control and auditability over encryption</li>\n'
            html += '<li><strong>LOW Priority:</strong> Enable deletion protection for critical tables to prevent accidental deletion</li>\n'
        html += '</ul>\n'
        html += '</div>\n'
...
def generate_html_report(verification_data):
    ...
    for section in verification_data.get('Sections', []):
        ...
        if 'VPC' in title:
            ...
            content += generate_dynamodb_section(section)
