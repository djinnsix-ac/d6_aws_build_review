...
def generate_vpc_section(vpc_data, collected_data=None):
    ...
    for vpc_id, checks in vpc_checks.items():
        ...
        # Prominent VPC heading with name
        html += f'''
        <div style="margin-top: 30px; padding: 15px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 8px;">
            <h3 style="color: white; margin: 0; font-size: 20px; font-weight: 600;">
                VPC: {vpc_name}
            </h3>
            <p style="color: rgba(255,255,255,0.9); margin: 5px 0 0 0; font-size: 14px;">
                <span class="code" style="background: rgba(255,255,255,0.2); padding: 3px 8px; border-radius: 4px;">{vpc_id}</span>
            </p>
        </div>
        '''
        ...
        html += '<h4 style="margin-top: 20px; color: #2c5aa0; border-bottom: 2px solid #2c5aa0; padding-bottom: 5px;">Configuration</h4>\n'
        ...
        # Subnet Distribution - Hierarchical view (collapsible)
        if subnet_info:
            subnet_id = f"subnets-{vpc_id}"
            html += f'''<h4 style="margin-top: 20px; color: #2c5aa0; border-bottom: 2px solid #2c5aa0; padding-bottom: 5px; cursor: pointer; display: inline-block;" 
                onclick="toggleSection('{subnet_id}')">
                <span id="{subnet_id}-icon" style="display: inline-block; width: 20px;">â–¼</span>
                Subnet Distribution
            </h4>\n'''
            ...
            html += f'<div id="{subnet_id}" style="padding: 15px; background: #f8f9fa; border-left: 4px solid #2c5aa0; border-radius: 4px; margin: 10px 0;">\n'
        ...
        # Route Tables summary (collapsible)
        if route_tables and raw_vpc:
            rt_section_id = f"route-tables-{vpc_id}"
            html += f'''<h4 style="margin-top: 20px; color: #2c5aa0; border-bottom: 2px solid #2c5aa0; padding-bottom: 5px; cursor: pointer; display: inline-block;" 
                onclick="toggleSection('{rt_section_id}')">
                <span id="{rt_section_id}-icon" style="display: inline-block; width: 20px;">â–¼</span>
                Route Tables
            </h4>\n'''
            
            html += f'<div id="{rt_section_id}">\n'
            html += '<table>\n'
            html += '<thead><tr><th>Route Table ID</th><th>Routes</th><th>IGW Route</th><th>NAT Route</th><th>Associations</th><th>Associated With</th></tr></thead>\n'
            html += '<tbody>\n'
            ...
            for idx, rt in enumerate(route_tables):
                ...
                if full_rt and full_rt.get('Associations'):
                    ...
                    for assoc in associations:
                        if assoc.get('Main'):
                            ...
                            assoc_list.append(f"Subnet: {assoc['SubnetId']}")
                ...
                html += f'''<tr>
                    ...
                    <td>{routes}</td>
                    <td>{"✓ Yes" if has_igw else "✗ No"}</td>
                    <td>{"✓ Yes" if has_nat else "✗ No"}</td>
                    <td>{assocs}</td>
                    <td style="font-size: 11px;">{assoc_details}</td>
                </tr>\n'''
        ...
        if raw_vpc:
            ...
            if vpc_endpoints:
                html += f'<h4 style="margin-top: 20px; color: #2c5aa0; border-bottom: 2px solid #2c5aa0; padding-bottom: 5px;">VPC Endpoints ({len(vpc_endpoints)})</h4>\n'
                html += '<table>\n'
        ...
        if raw_vpc:
            ...
            if peering_connections:
                html += f'<h4 style="margin-top: 20px; color: #2c5aa0; border-bottom: 2px solid #2c5aa0; padding-bottom: 5px;">VPC Peering Connections ({len(peering_connections)})</h4>\n'
                html += '<table>\n'
