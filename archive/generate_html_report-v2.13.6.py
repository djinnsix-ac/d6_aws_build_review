...
def generate_compute_section(compute_data):
    ...
    if compute_data.get('ecs'):
        ...
        for cluster in compute_data['ecs']:
            ...
            html += f'<h4>Cluster: {cluster_name}</h4>\n'
...
def generate_load_balancer_section(lb_data):
    ...
    if albs:
        ...
        for lb in albs:
            ...
            azs = ', '.join(lb.get('AvailabilityZones', []))
            ...
            tg_text = '<br>'.join([f"{tg.get('Name')} ({tg.get('HealthyTargets', 0)}/{tg.get('TotalTargets', 0)} healthy)" for tg in tgs])
